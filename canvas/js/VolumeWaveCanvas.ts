class VolumeWave{
    ...
    charCount:number
    //二维数组
    charArrList:Array<Array<number>>
    ctx:CanvasRenderingContext2D|null
    constructor(parentId:string,canvasId:string,charCount:number = 1000){
    ...
    */
    init(){
        ...
        var canvasArea:any = document.getElementById(this.parentId)
     ...
     * @return {void}
    */
    setVolumeVal(valueArr:Array<Array<number>>){
        this.charArrList[0].push(...valueArr[0])
        this.charArrList[1].push(...valueArr[1])
        var canvasArea:any = document.getElementById(this.parentId)
        let canvasHalfHeight = canvasArea.offsetHeight / 2
        //重置画布
        this.ctx.clearRect(0, 0, canvasArea.offsetWidth, canvasArea.offsetHeight);
        //如超过长度则去除
        this.charArrList.map(char=>{
            if(char.length>this.charCount){
                char.splice(0,char.length-this.charCount)
            }
        })
        for (let i = 0; i < this.charArrList[0].length; i++) {
            this.ctx.fillStyle = '#aaaa00'
            //从中向下
            let bottomHeight = volumeScaleMap[this.charArrList[1][i]]
            this.ctx.fillRect(
                i,
                canvasHalfHeight,
                1,
                (canvasHalfHeight/20)*bottomHeight
            );
            //从中向上
            let topHeight = volumeScaleMap[this.charArrList[0][i]]*-1
            this.ctx.fillStyle = '#009600'
            this.ctx.fillRect(
                i,
                canvasHalfHeight,
                1,
                (canvasHalfHeight/20)*topHeight
            );
